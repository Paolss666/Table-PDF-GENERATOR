...
def _build_retro_page(
    retro_png: bytes,
    table_number: str,
    wr_code: str,
    table_number_y: float = TABLE_NUMBER_Y,
    qr_y: float = QR_Y,
) -> bytes:
    ...
    img.paste(qr_img, (cx - qr_size // 2, int(h * qr_y) - qr_size // 2), mask=qr_img.split()[3])
...
def generate_tables_pdf(
    ...
    table_number_y and qr_y are vertical positions as a fraction of image height (0.0–1.0).
    ...
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(_build_retro_page, retro_png_base, tn, wc, table_number_y, qr_y): idx
